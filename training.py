...
def train_exhaustive(datasets=["hmda"], save_dir="results/exhaustive/"):
    n_folds = 5
    for dataset in datasets:
        dataset = get_dataset(dataset)
        pf = ParetoFront(protected_col=dataset.protected_col, models=ModelSets.best_models)
        lambds = [0.2, 0.5, 0.7]
        rs = RegularizedSelection(protected_col=dataset.protected_col, models=ModelSets.best_models, lambd=lambds)
        lambds = [0.2, 0.5, 0.7]
        dcs = [DecoupledClassifier(protected_col=dataset.protected_col, name=f"Decoupled_Classifier_{lambd}",
                                   models=[RandomForestClassifier(), XGBClassifier(use_label_encoder=False,
                                                                                   eval_metric='logloss')],
                                   lambd=lambd) for lambd in lambds]
        other_models = []
